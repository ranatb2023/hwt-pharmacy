...
export const BTN = 'h-9 px-3 bg-white hover:bg-slate-50 border border-slate-300 rounded-md font-semibold text-slate-700 inline-flex items-center gap-1.5 text-xs whitespace-nowrap disabled:opacity-40 disabled:cursor-not-allowed shadow-sm';
export const BTN_PRIMARY = 'h-9 px-3.5 bg-[#0284c7] hover:bg-blue-600 text-white rounded-md font-bold inline-flex items-center gap-1.5 text-xs whitespace-nowrap disabled:opacity-40 disabled:cursor-not-allowed shadow-sm';
export const BTN_DARK = 'h-9 px-3.5 bg-[#0b1f3d] hover:bg-[#122e54] text-white rounded-md font-bold inline-flex items-center gap-1.5 text-xs whitespace-nowrap disabled:opacity-40 disabled:cursor-not-allowed';
export const BTN_SM = 'h-7 px-2.5 bg-white hover:bg-slate-50 border border-slate-300 rounded font-semibold text-slate-700 inline-flex items-center gap-1 text-[11px] whitespace-nowrap disabled:opacity-40 disabled:cursor-not-allowed';
...
export function Kbd({ children }) {
  return <kbd className="text-[10px] font-mono font-bold bg-white/20 px-1.5 py-0.5 rounded border border-white/30">{children}</kbd>;
}
...
export function Kpi({ label, value, unit, sub, tone = 'slate', icon, mono = true }) {
  ...
  return (
    <div className="bg-white rounded-lg border border-slate-200 p-5 shadow-sm">
      ...
      <div className={`text-3xl font-extrabold mt-1 leading-tight ${val} ${mono ? 'font-mono tracking-tight' : 'font-headline'}`}>
    ...
    <div className="flex items-center gap-1.5 flex-wrap">
      {items.map(([k, label, n]) => (
        <button key={k} type="button" onClick={() => onChange(k)}
          className={`h-8 px-3 rounded-md text-xs font-semibold border transition-colors ${value === k ? 'bg-[#0284c7] text-white border-[#0284c7]' : 'bg-slate-50 text-slate-700 border-slate-200 hover:bg-slate-100'}`}>
          {label}{n != null ? ` (${n})` : ''}
        </button>
      ))}
    </div>
...
export function Search({ value, onChange, placeholder, kbd, className = '' }) {
  return (
    <div className={`relative ${className}`}>
      ...
      {kbd && <div className="absolute inset-y-0 right-0 pr-3 flex items-center"><span className="text-[10px] font-mono font-bold text-slate-500 bg-slate-100 border border-slate-200 px-1.5 py-0.5 rounded">{kbd}</span></div>}
    </div>
  );
}
...
export function Tbl({ head, right = [], children, dense }) {
  return (
    <div className="overflow-x-auto">
      <table className="w-full text-xs border-collapse">
        ...
        <tbody className="divide-y divide-slate-100 [&>tr>td]:px-4 [&>tr>td]:py-3 [&>tr:hover]:bg-slate-50/70">{children}</tbody>
      </table>
    </div>
  );
}
