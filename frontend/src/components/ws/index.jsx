  ...
  return (
    <header className="bg-white border-b border-slate-300 shadow-xs shrink-0 z-20 no-print">
      <div className="px-4 py-2 flex items-center justify-between gap-4">
        {/* Brand + counter */}
        <button type="button" onClick={() => nav('/')} className="flex items-center gap-2.5 text-left group"
          title="Home">
          <img src="/img/Hope-Charity-Logo.webp" alt="" className="h-8 w-8 object-contain" />
          <div>
            <div className="flex items-center gap-2">
              <span className="font-bold text-slate-900 text-sm tracking-tight">HOPE WELFARE TRUST</span>
              <span className="text-[11px] font-semibold text-slate-600 bg-slate-100 border border-slate-300 px-1.5 py-0.5 rounded">
        ...
        <div className="flex items-center gap-3">
          <div className="flex items-center gap-2 border-r border-slate-200 pr-3">
            ...
            <div className="h-7 w-7 rounded bg-slate-200 text-slate-700 font-bold text-xs flex items-center justify-center border border-slate-300">
              {(user?.full_name || '?').split(' ').filter(Boolean).slice(0, 2).map((s) => s[0].toUpperCase()).join('')}
            </div>
          </div>
          {tillCash != null && (
            <div className="hidden sm:flex flex-col text-right">
              <span className="text-[10px] text-slate-500 font-semibold uppercase">Till cash</span>
              <span className="text-xs font-mono font-bold text-slate-900">{rs(tillCash)}</span>
            </div>
          )}
          <div className="flex items-center gap-1.5">
            <button type="button" onClick={() => setTheme(toggleTheme())} title={theme === 'night' ? 'Day mode' : 'Night mode'}
              className="px-2 py-1 text-xs font-medium text-slate-700 bg-white border border-slate-300 hover:bg-slate-50 rounded">
              {theme === 'night' ? '☀' : '☾'}
            </button>
            {dayClose && (
              <button type="button" onClick={() => nav('/pharmacy-close')}
                className="px-2.5 py-1 text-xs font-medium text-slate-700 bg-white border border-slate-300 hover:bg-slate-50 rounded flex items-center gap-1.5"
                title="Day-end close">
                Day Close <Kbd>F10</Kbd>
              </button>
            )}
            <button type="button" onClick={logout}
              className="p-1.5 text-slate-500 hover:text-rose-700 hover:bg-rose-50 border border-slate-200 rounded"
              title="Sign out">
...
// ---------------------------------------------------------------------------
export const DOCK = [
  { to: '/pharmacy', label: 'POS', perm: 'pharmacy.sell', title: 'Main POS' },
  { to: '/departments', label: 'Requisitions', perm: 'pharmacy.dispense', title: 'Department requisitions & ward indents' },
  ...
  { to: '/customers', label: 'Customers', perm: 'patient.view', title: 'Customers & welfare cards' },
...
export const MORE = [
  ...
  { to: '/dialysis', label: 'Dialysis Unit', perm: 'dialysis.view' },
  { to: '/margin', label: 'Profitability', perm: 'report.view' },
...
export function WsNav() {
  ...
  return (
    <nav className="bg-slate-50 border-b border-slate-300 px-3 flex items-center gap-1 shrink-0 overflow-x-auto no-print">
      {dock.map((d, i) => (
        <NavLink key={d.to} to={d.to} title={`${d.title || d.label} (Alt+${i + 1})`}
          className={({ isActive }) =>
            `flex items-center gap-1.5 px-2.5 py-2 text-xs font-semibold whitespace-nowrap border-b-2 -mb-px transition-colors ${
              isActive
      ...
      {more.length > 0 && (
        <div className="relative ml-auto">
          <button type="button" onClick={() => setOpen((v) => !v)} aria-expanded={open} aria-haspopup="menu" aria-controls="ws-more-menu"
            className="flex items-center gap-1 px-3 py-2 text-xs font-semibold text-slate-600 hover:text-slate-900">
            More <span className="text-[10px]">▾</span>
          </button>
          {open && (
            <div id="ws-more-menu" role="menu" className="absolute right-0 top-full mt-1 bg-white border border-slate-300 rounded-md shadow-lg z-30 min-w-[180px] py-1"
              onMouseLeave={() => setOpen(false)}>
              {more.map((d) => (
                <NavLink key={d.to} to={d.to} end={d.to === '/'} onClick={() => setOpen(false)}
                  className={({ isActive }) =>
                    `block px-3 py-1.5 text-xs ${isActive ? 'text-clinical-primary font-semibold' : 'text-slate-700 hover:bg-slate-50'}`}>
                  {d.label}
                </NavLink>
              ))}
            </div>
